...
class _MainViewState extends ConsumerState<MainView> {
  ...
  List<Category> _filterCategories(AppState appState) {
    return appState.categories.where((category) => 
      _searchQuery.isEmpty || category.name.toLowerCase().contains(_searchQuery.toLowerCase())
    ).toList();
  }

  Sidebar _buildSidebar(MacosThemeData theme, AppState appState, List<Category> filteredCategories) {
    return Sidebar(
  ...
  Expanded _buildCategoryList(AppState appState, List<Category> filteredCategories, ScrollController scrollController) {
    return Expanded(
  ...
  Padding _buildCategoryItem(Category category, bool isSelected, AppState appState) {
    final theme = MacosTheme.of(context);
    return Padding(
      ...
      child: Container(
        ...
        child: GestureDetector(
          onSecondaryTapUp: (details) => _showCategoryContextMenu(context, category, details.globalPosition),
          child: Row(
            children: [
              Expanded(
                child: MacosListTile(
  ...
  MacosIconButton _buildCategoryOptionsButton(BuildContext context, Category category, MacosThemeData theme) {
  ...
  Padding _buildSettingsOption(MacosThemeData theme, int pageIndex) {
    return Padding(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: _pageIndex == pageIndex ? theme.primaryColor.withOpacity(0.2) : Colors.transparent,
          borderRadius: BorderRadius.circular(8.0),
        ),
        child: MacosListTile(
          leading: MacosIcon(
  ...
  Widget _buildBottomSidebar(AppState appState, MacosThemeData theme) {
    return Column(
      ...
      children: [
        ...
        _buildCreateCategoryButton(appState, theme),
      ],
    );
  }

  Padding _buildCreateCategoryButton(AppState appState, MacosThemeData theme) {
    return Padding(
      padding: const EdgeInsets.all(12.0),
      child: PushButton(
